...
class BirdCubit extends Cubit<BirdState>{
  ...
  Future<void> getBirds({String? name, String? categoryId, String? speciesId, int? pageNumber, int? pageSize}) async {
    emit(BirdLoadingState());
    try {
      var birds = await _birdRepo.getBirds(name: name, categoryId: categoryId, speciesId: speciesId, pageNumber: pageNumber, pageSize: pageSize);
